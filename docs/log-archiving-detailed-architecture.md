...
An operator can also add multiple custom tables or views with a configured timestamp column. A single job can archive several built-in and custom sources during one scheduled execution.
...
- source/archive hosts, ports, sockets, usernames, archive database/table;
- source selections, custom source definitions, schedule, retention, and batch size;
...
| `log_type` | Built-in source type or custom-source identity |
...
The timestamp cursor is advanced only after source rows have been processed. Operators should ensure custom timestamp columns are suitable for monotonically progressing extraction. If a source produces more than one batch with identical timestamps, a source-specific immutable key should be included in a future cursor enhancement to guarantee no timestamp-tie gaps.
...
| Job configuration | Enable/disable, schedule, retention, multi-select built-in logs, OCI Vault OCIDs, source/archive connections, custom source maintenance |
...
Archive entries can be narrowed by archive source and monthly partition. The report table supports filter, page sizing, CSV download, layout reset, column sorting, resizing, reordering, and full payload viewing.
